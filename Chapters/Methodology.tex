...
\ac{DQN} and \ac{A3C} algorithms get less than $100$ score on average when playing this game.
...
\begin{figure}[hbtp]
\begin{center}
\includegraphics[width=300]{img/montezuma.png}
\end{center}
\caption[Montezuma's Revenge game]
{First screen of \acl{MR} game.
The hero is in the starting position of the game.}
...
\begin{itemize}
    ...
    \item The hero have multiple lives but I will consider he has just $1$ and, if he dies, I will reset the game.
    \item When the hero reaches the key I will also reset the game.
    So the unique goal of the hero is to take that object.
\end{itemize}
% TODO HABLAR SOBRE EL ALE o lo que use
% TODO USO LA MEMORIA PARA OBTENER LA POSICIÓN DEL PERSONAJE
The mechanics remain the same than in the original game.
...
There are two things which the hero can die.
The first one is by falling into the floor from some high position, for example from the rope.
The second one is by touching the skull in the bottom, which is always moving left and right.

The hero can gain score by collectig the key, which gives to him 100 points.
He can also gain score by going through the checkpoints... %TODO PONER EN LA IMAGEN LOS CHECKPOINTS
There are also
