...
I have modified the \ac{A3C} algorithm in order to take advantage of sub-tasks with the purpose of enhancing training speed
and exploration.
I refer to this new algorithm as \acf{MA3C} and in this section you will find specifications about the architectures of
both \acp{ANN}: \ac{A3C} and \ac{MA3C}.
In addition I will explain the \ac{MA3C} algorithm.

They have been developed using tensorflow (\cite{tensorflow2015}), an open source machine learning framework which I find %TODO evita dar opiniones, pon datos de papers mejor
really useful to implement deep learning algorithms.

\subsection{\acl{A3C}\label{subsec:AlgorithmA3C}}

As I have explained previously (\ffref{subsec:A3C}), this algorithm uses the image of the environment to represent the state.
...
The algorithm applies this preprocessing to 4 stacked frames allowing the neural network to implicitly calculate the movement %TODO cambiar todo Neural Networks por NN
...
different depending on the problem or the use case. %TODO no se entiende este or the use case
...
with a couple of rules or just be a piece of the state. %TODO la ultima parte or just be a piece of the state no tiene sentido leyendo la frase completa
For this thesis I have used two approaches: part of the state, in Simple States (\ffref{subsec:SimpleStates}) and Complex States
...
The full algorithm with the few changes explained is shown in \ffref{alg:MA3C}. %TODO yo diria The full modified algorithm
...
to the player. %TODO reescribe este parrafo entero
...
checkpoint, door. %TODO quizas representaria el orden con flechas o otra cosa en vez de comas
...
%TODO COMPROBAR SI ESTA ESCRITO MONTEZUMAS REVENGE SIN ACRONIMO
...
Since this game is really complex and computationally expensive to train, I have made some changes to it.
\begin{itemize}
    \item In the hole game there are several screens but I will only use the first one in my experiments.
    \item The hero has multiple lives but I will consider he has just $1$ and, if he dies, the game will be reset.
    \item When the hero reaches the key I will also reset the game, so the unique goal of the hero is to take that object.
\end{itemize}
...
I will use later the position (in screen space) of the objects, which are the following:
\begin{itemize}
    ...
    \item key: $(14, 209)$
\end{itemize}
In order to obtain this positions, and the agent's position, I have used the memory layout described in \citetitle{adriaTFG}
(\cite{adriaTFG}).
...
you change to the next, it is impossible to go back to another previous option/phase. %TODO tenias puesto its, no poner contracciones nunca
